...
public class FileManager implements FileModelInterface {
  ...
  public boolean aggiungiFile(File c) {
    ...
    try {
      ...
      prepared = connection.prepareStatement(
          "INSERT INTO FILE (NOME,PROPOSTATESI_ID,FILE,DESCRIZIONE,UTENTE_EMAIL) "
          + "VALUES (?,?,?,?,?)");
  ...
  public boolean modificaFile(int idTesi, String nomeFile, int voto, String descrizioneVoto) {
    ...
    try {
      connection = DatabaseManager.getIstance();
      prepared = connection.prepareStatement("UPDATE FILE SET Voto=?,Descrizione_Voto=?  "
          + "Where Nome=? AND PropostaTesi_Id=?");
  ...
  public File getFile(int id, String nomeFile) {
    try {
      connection = DatabaseManager.getIstance();
      prepared = connection.prepareStatement("SELECT * FROM FILE "
          + "WHERE PROPOSTATESI_ID=? AND NOME=?");
